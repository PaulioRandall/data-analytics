...
1. Does population density correlate with Covid spread within a country?
...
## Does population density correlate with Covid spread?

This visualisation aims to identify if population density is correlated with
Covid spread. Note that this does not mean that population density is a key
factor in transmission rates; although, it probably is. I will use new Covid
cases per week as a measure of spread without attempting to account for other
variables.
...
typed_data %>%
  weekly_sum() %>%
  select(population_density, new_cases_per_million) %>%
  filter(
    year == 2021,
    week == 1,
    new_cases_per_million > 0,
    new_cases_per_million < ONE_HUNDRED_MILLION,
    population_density > 0,
    population_density < (ONE_MILLION * 0.75),
  ) %>%
  mutate(
    new_cases_per_million_in_millions = new_cases_per_million / ONE_MILLION,
  ) %>%
  print() %>%
  ggplot() +
  geom_point(
    mapping = aes(
      y = new_cases_per_million_in_millions,
  ...
  labs(
    title="Does population density correlate with Covid spread?",
    subtitle="Where population density < 750,000",
    y="New cases (per million)",
    x="Population density (People per km2)",
  )

save_plot("does-population-density-correlate-with-covid-spread.jpeg")
