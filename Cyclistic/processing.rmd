...
(probably SQLite to begin with) to better deal with the volume of data.

## Assignment

In a nutshell, Cyclistic a made up bike hiring company operating in Chicago has
determined that annual membership holders are more profitable than casually
...
base including how casual riders differ in behaviour and usage of Cyclistic's
bikes. They have tasked me with using their past years anonymised data on
bicycle usage to garner insights and generate recommendations on how to proceed.

The 2021 data is actually real data made publically available by a bike share
...
A quick modifiable snippet for general inspection of the raw trip data before
any cleaning and manipulation.
...
possible to find identify trends in missing station details from this data set
but a full analysis is out of scope.

In regards to the task at hand, the missing data and removal of associated
entries may cause an unintended bias in results and conclusions which may then
result in sub-optimal decision making. This would be especially true if the
issues are generalized around specific stations or bike types as these
variables are analysed and visualized towards the end of this document.
...
may prove useful for ingesting into other visualisation tools later.
...
the mean for each rider type is calculated from all cleaned trips.
